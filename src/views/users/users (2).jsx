...
const UserInfo = () => {
    ...
    const handleUnFollowUser = (info) => {
        fetch(`${process.env.PROJECT_HOST}/users/${Cookies.get("user")}/unfollow/${info.user.username}/`)
            .then(response => response.json())
            .then((resposne) => {
                if (response.state == "successfully") {
                    ...
                    setBtn([
                        Cookies.get("user") != info.user.username && !state.followed &&
                        <Button
                            className="button collection-user"
                            onClick={((info) => {
                                return () => {
                                    return handleFollowUser(info);
                                }
                            })(info)}
                        >
                            {"关注"}
                        ...
                        <Button
                            ...
                            onClick={((info) => {
                                return () => {
                                    return handleUnFollowUser(info);
                                }
                            })(info)}
                        >
                            {"取消关注"}
    ...
    const handleFollowUser = (info) => {
        fetch(`${process.env.PROJECT_HOST}/users/${Cookies.get("user")}/follow/${info.user.username}/`)
            .then(response => response.json())
            .then((resposne) => {
                if (response.state == "successfully") {
                    setState({
    ...
    if (!stop) {
        ...
        fetch(`${setting.base}api/session/`, {
            ...
            .then(data => {
                ...
                setContent([c]);
                fetch(`${process.env.PROJECT_HOST}/users/${Cookies.get("user")}/followed/${info.user.username}/`)
                    .then(response => response.json())
                    .then((response) => {
                        setState({
                            followed: response.followed
                        });
                        state.followed = response.followed;
                        setBtn([
                            Cookies.get("user") != info.user.username && !state.followed &&
                            <Button
                                className="button collection-user"
