    ...
    if (!stop) {
        ...
        fetch(`${setting.base}api/session/`, {
            ...
            .then(data => {
                ...
                a.current.href = `/wait_projects/${info.user.username}/`;
...
const WaitProjects = () => {
    ...
    var timer = setInterval(function (){
        ...
        fetch(`${process.env.PROJECT_HOST}/scratch-admin/projects/not_passed/${p_info.type}`,{
            method: "POST"
        })
            .then(response => response.json())
            .then(data => {
                setContent([
                    <h3>等待审核的页面 - {{ "all": "全部", "unchecked": "未检查的", "checked": "已检查但未通过的" }[p_info.type]}({data.length})</h3>
                    ,
                    <div>
                        {data.length > 0 ? 
    ...
    const getLinks = () => {
        var ret = [];
        var pages = { "all": "全部", "unchecked": "未检查的", "checked": "已检查但未通过的" };
    ...
    return (
        <div className="inner wait_projects" id="pagebox">
            <Box
                headContent={
                    <UserInfo key="page-user-info-title" />
                }
...
render(<Page><WaitProjects /></Page>, document.getElementById('app'));
